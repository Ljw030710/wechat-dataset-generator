...
生成、渲染与导出由独立命令完成。保存的会话 JSON 是后续处理的共同输入，可以修改渲染参数后重新出图，或单独重新导出标签。
...
模型审校与生成使用同一个客户端，区别在于提示词和阶段职责。一次顺利完成的生成需要四次模型调用，修复与重新生成会增加请求数量。
